...
class ThreshNet(nn.Module):
    def __init__(self):
        ...
        ch_list  = [  128, 256, 384, 480, 960]
        gr       = [  32, 32, 32, 40, 160]
        n_layers = [   6, 12, 16, 16,  4]
        downSamp = [   1,  1,  0,  1,  0]
        ...
        #Second ThreshBlock
        self.base.append ( ThreshLayer(ch, 32, kernel=3, stride=1,  bias=False) )
        for i in range (160, 512, 32):
          self.base.append ( ThreshLayer(i, 32, kernel=3, stride=1,  bias=False) )                                
        self.base.append ( ConvLayer(512, ch_list[1], kernel=1) )
        ch = ch_list[1]
        if downSamp[1] == 1:
          self.base.append(nn.MaxPool2d(kernel_size=2, stride=2))
        ...
        #Third ThreshBlock
        self.base.append ( ThreshLayer(ch, 32, kernel=3, stride=1,  bias=False) )
        for i in range (288, 768, 32):
          self.base.append ( ThreshLayer(i, 32, kernel=3, stride=1,  bias=False) )               
        self.base.append ( ConvLayer(768, ch_list[2], kernel=1) )
        ch = ch_list[2]
        if downSamp[2] == 1:
          self.base.append(nn.MaxPool2d(kernel_size=2, stride=2))
